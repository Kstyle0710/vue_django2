      ...
      <v-col cols="12" sm="4" lg="3">
        ...
        <v-card class="pa-2 mb-5" tile>
          ...
          <v-chip
            ...
            text-color="white"
          >
            <v-avatar left :class="tag.color + ' green darken-4'">{{ tag.count }}</v-avatar>
            {{ tag.name }}
          </v-chip>
        </v-card>
      </v-col>
...
export default {
  ...
  methods: {
    fetchPostDetail(postId) {
      ...
      axios
        .get(`/api/post/${postId}/`) // ES6 template literal 사용
        .then((res) => {
    ...
    fetchTagCloud() {
      ...
      axios
        .get('/api/tag/cloud/') // ES6 template literal 사용
        .then((res) => {
          ...
          this.tagCloud.forEach(element => {
            ...
            else if (element.weight === 2) element.color = 'brown';
